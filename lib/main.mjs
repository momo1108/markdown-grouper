...
class MarkdownDocument {
    ...
    treeSearch(headerComponent, prefix){
        // console.log(headerComponent.innerText, depth, isEnd);
        let headerText = `${prefix}${headerComponent.innerText}\n`;

        for(let child = 0; child < headerComponent.children.length; child++){
            let isLastchild = child === (headerComponent.children.length - 1);
            let childPrefix = `${prefix}  ${isLastchild?" └":" ├"}`;
            let childText = this.treeSearch(headerComponent.children[child], childPrefix);
