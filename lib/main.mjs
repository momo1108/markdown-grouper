...
class MarkdownDocument {
    ...
    showHeaderTree(){
        ...
        for(let i = 0; i < this.#roots.length; i++){
            totalHeaderText += this.treeSearch(this.#roots[i], []);
        }
    ...
    treeSearch(headerComponent, treeInfo){
        // console.log(headerComponent.innerText, depth, isEnd);
        let headerText = treeInfo.reduce((prev, cur, curIndex)=>{
            if(curIndex===(treeInfo.length-1)){
                return prev + (cur? "  └" :  "  ├");
            } else {
                return prev + (cur? "   " :  "  │");
            }
        }, "") + `${headerComponent.innerText}\n`;

        for(let child = 0; child < headerComponent.children.length; child++){
            let isLastchild = child === (headerComponent.children.length - 1);
            headerText += this.treeSearch(headerComponent.children[child], [...treeInfo, isLastchild]);
        }
 ...
 */
export const markdownDocument = new MarkdownDocument();
...
export const parseToGroup = (markdownText, minLevel=1, selector="id", prefix="_", postfix="-") => {
    ...
    markdownDocument.setDocument(markdownText, minLevel, 1, "", selector, prefix, postfix);
