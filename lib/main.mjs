...
class MarkdownDocument {
    ...
    treeSearch(headerComponent){
        // console.log(headerComponent.level, headerComponent.innerText);
        let headerText = `${"  ".repeat(headerComponent.level-1)}${headerComponent.level>1?"├":""}${headerComponent.innerText}\n`;

        for(let child = 0; child < headerComponent.children.length; child++){
            let childText = this.treeSearch(headerComponent.children[child]);
